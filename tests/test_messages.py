...
from pydantic import BaseModel, field_validator, version
...
class Example(BaseModel):
    ...
    @field_validator("ipt_enum", mode="after")
    @classmethod
    ...
    data = {
        ...
        "nested": {
            ...
            "inner": {
                ...
                "cat": {"pet_type": "cat", "color": "black", "black_name": "string"},
...
def test_i18n_validation():
    data = {
        ...
        "nested": {
            ...
            "inner": {
                ...
                "cat": {"pet_type": "dog", "color": "black", "black_name": "string"},
    ...
    assert list(translate_errors(translator, exc_info.value.errors())) == [
        {  # type: ignore
            ...
            "input": "dog",
            "ctx": {"expected": "'cat'"},
            "url": f"https://errors.pydantic.dev/{pydantic_version}/v/literal_error",
        },
        {
            "type": "too_long",
            ...
            "input": "dddddddd",
            "ctx": {"field_type": "Value", "max_length": 5, "actual_length": 8},
            "url": f"https://errors.pydantic.dev/{pydantic_version}/v/too_long",
        },
    ]
